...
class ValidateRenameColumns(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_rename_dict_keys_in_df_columns(self
    ) -> Self:
        df = self.df
        _rename_dict = self.cols_to_rename
...
class ValidateGroupbyAndAgg(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_groupby_agg_columns_in_df_columns(self) -> Self:
        
        df = self.df
        _cols = self.cols
        _dict_agg = self.agg_dict
